...
create_recipe <- function(data) {
 ...
 recipe <- create_feature_engineerging_recipe(data)
  ...
  recipe %>%
    ...
    # 標準化
    recipes::step_normalize(recipes::all_numeric(), -recipes::all_outcomes(), -id) %>%
...
# special_segment を追加
add_special_segment <- function(data) {
  ...
  recipes::recipe(~ ., data) %>%
    ...
    recipes::step_mutate(
      special_segment = dplyr::case_when(
        ...
        comment_count < -0.90 ~ "ratings_abled_low_comments",
        T                     ~ "others"
      ) %>%
    ...
    recipes::step_rm(special_segment_tmp) %>%
